$p = "app/dashboard-new/ai-chat/page.tsx"
$c = @"
"use client"
      ...
      <div className="flex bg-gray-950 text-gray-100" style={{ height: "100vh" }}>
        <div className={`${sidebar ? "w-60" : "w-0"} transition-all duration-200 overflow-hidden flex-shrink-0 border-r border-gray-800 flex flex-col bg-gray-900`}>
          <div className="p-3 border-b border-gray-800">
            <button onClick={newChat} className="w-full py-2 px-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium">New chat</button>
          </div>
          {ks && (
            <div className="px-3 py-2 border-b border-gray-800">
              ...
              <div className="flex flex-wrap gap-1">
                {[{ l: "Groq", ok: ks.groq }, { l: "OpenAI", ok: ks.openai }, { l: "Gemini", ok: ks.gemini }].map(p => (
                  <span key={p.l} className={`text-xs px-2 py-0.5 rounded-full ${p.ok ? "bg-green-900 text-green-300" : "bg-gray-800 text-gray-500"}`}>{p.ok ? "v" : "x"} {p.l}</span>
                ))}
              </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {sessions.length === 0
              ? <p className="text-xs text-gray-600 text-center mt-6 px-3">Sorry, curently haven't last session.</p>
              : sessions.map(s => (
...
Write-Host "Done: $((Get-Item $p).Length) bytes"
