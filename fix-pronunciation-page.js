...
// Parse AI response to extract mispronounced words (format: /word/ in text)
function extractMispronouncedWords(text: string): string[] {
  const matches = text.match(/\\/([a-zA-Z]+)\\//g) || []
  return [...new Set(matches.map(m => m.replace(/\\//g, "")))]
}

// Render message with clickable pronunciation buttons for mispronounced words
        ...
        <div className="mt-3 pt-3 border-t border-violet-100">
          ...
          <div className="flex flex-wrap gap-2">
            {words.map(word => (
              ...
              >
                <svg className="w-3.5 h-3.5 text-violet-600 group-hover:text-violet-800" fill="currentColor" viewBox="0 0 24 24">
...
export default function PronunciationPage() {
  ...
  // Speak a single word with clear, slow pronunciation
  const speakWord = useCallback((word: string) => {
    window.speechSynthesis?.cancel()
    setSpeakingWord(word)
    const utt = new SpeechSynthesisUtterance(word)
    utt.lang = "en-US"
    utt.rate = 0.7 // Slower for clarity
    utt.pitch = 1.1
      ...
      <div className="flex flex-col h-screen bg-gradient-to-br from-violet-50 via-white to-indigo-50">
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-white/80 backdrop-blur-sm border-b border-violet-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500">Speak English — get feedback + hear correct pronunciation</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <p className="text-gray-400 text-xs max-w-xs mb-6">If you mispronounce a word, I'll show it with a <span className="bg-violet-100 text-violet-700 px-1.5 py-0.5 rounded-full text-xs font-medium">🔊 /word/</span> button so you can hear the correct pronunciation.</p>
          ...
          {messages.map((m, i) => (
            <div key={i} className={\`flex \${m.role === "user" ? "justify-end" : "justify-start"}\`}>
              {m.role === "user" ? (
                <div className="max-w-[80%] bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-2xl rounded-br-sm px-4 py-3 text-sm shadow-md">
                  {m.content}
                </div>
              ) : (
